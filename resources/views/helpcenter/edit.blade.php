...
<form method="post" action="{{ route('helpcenter.update',['helpcenter'=> $value->id]) }}">
    @csrf
    @method('PATCH')
    <div class="card mb-4">
        <h5 class="card-header text-left">Add FAQ</h5>
        <hr class="my-0"/>
        <div class="card-body">
            <div class="row">
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <textarea name="question"  id="question" class="form-control " rows="8">{{$value->question}}</textarea>
                        
                    </div>
                    <div class="col-md-6">
                    <label for="answer" class="form-label">Answer</label>
                    <textarea name="answer"  id="" class="form-control ckeditor" rows="8">{{$value->answer}}</textarea>
                </div>
                </div>
            </div>
